...
Exclusion lists identify areas of the genome that have low mappability rates or contain high artifactual signals. We used `excluderanges v 0.99.8` to download the exclusion list for TAIR10 defined by the [Greenscreen](https://academic.oup.com/plcell/article/34/12/4795/6705244) pipeline. Reads mapping to these regions are removed ChIP-seq peak-calling software such as MACS3. 
...
We used `annotatePeaks.pl` from [HOMER v5.1](http://homer.ucsd.edu/homer/ngs/annotation.html) to identify the nearest gene to each peak, and to find instances of the supplied motif sequence within each peak.
...
We used the [`streme`](https://meme-suite.org/meme/doc/meme.html?man_type=web) and [`sea`](https://meme-suite.org/meme/doc/sea.html?man_type=web) from [MEME v5.5.4](https://meme-suite.org/meme/index.html) to discover novel, de novo motifs and find enrichment for known motifs, respectively.

Known motifs came from DNA affinity purification sequencing (DAP-seq), a high-throughput TF binding site discovery method that interrogates genomic DNA with in-vitro-expressed TFs (O'Malley et al., Cell, 165:1280-1292, 2016).
 ...
 * `FDR`: multiple-testing corrected pvalue calculated by `DESeq2`        
...
We used [`topGO v2.54.0`](https://bioconductor.org/packages/release/bioc/html/topGO.html) with Fisher's exact test and the gene ontoloy annotations from the [`org.At.tair.db v 3.18.0`](https://bioconductor.org/packages/release/data/annotation/html/org.At.tair.db.html) bioconductor annotation package to check for enrichment of genes related to specific biological processes, molecular functions, and cellular compartments among genes near ChIP peaks. Only GO terms with 5 > n genes > 200 were included. GO terms containing several hundred genes are often overly broad and, therefore, uninformative. 
