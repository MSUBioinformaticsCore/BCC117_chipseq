...
## Find overlapping peak sets

Compare up to 5 sets.
...
1. Path to sample sheet 
2. Path to results dir 
3. Path to `peakUpset.R`

**Output**
* `overlaps/*_shared_peaks.csv` files listing shared and unique peaks
* `overlaps/*_upset.pdf` plots showing the number of unique and overlapping peaks 

```{bash}
cd $PROJECT/run

sbatch $SRC/src/run_peakUpset.sh \
  $PROJECT/data/overlap_sample_sheet.csv \
  $PROJECT/results \
  $SRC/src/peakUpset.R
```
