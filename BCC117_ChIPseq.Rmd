...
The exclusion lists identify areas of the genome that have low mappability rates or contain high artifactual signals. Reads mapping to these regions should be masked out before applying ChIP-seq peak-calling software such as MACS2. `getExclusionList.R` uses `excluderanges v 0.99.8` to download the exclusion list for TAIR10 defined by the [Greenscreen](https://academic.oup.com/plcell/article/34/12/4795/6705244) pipeline. Adds Mt and Pt chromosomes to blacklist.
...
Rscript $PROJECT/src/makeDiffBindSampleSheet.R \
  ...
  $PROJECT/results/bwa/mergedLibrary/macs3/narrow_peak \
  $PROJECT/data
...
sbatch $PROJECT/src/runDiffBind.sh \
  $PROJECT/data/sample_sheets/DiffBind_samplesheet.csv \
  $PROJECT/results \ 
  TRUE \
